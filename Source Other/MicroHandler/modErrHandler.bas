...
Public Function MyExceptionFilter(ByRef ExceptionPtrs As EXCEPTION_POINTERS) As Long
    ...
    Err.Raise 10000, "Exception Error ", strException
End Function

Public Sub GestioneErrore(Numero As Integer, Sorgente As String, Dove As String, Descrizione As String)
        DisplayError (Numero & vbCrLf & Sorgente & _
                vbCrLf & Dove & vbCrLf & Descrizione)
End Sub
...
On Error GoTo ErrH
    ...
    ErrorNum = xmMP3_getLastErrorNo ' + GetErrorDescription(ErrorNum)
    If xmp.WindowState <> vbNormal Then
        ...
        Listone.Show
    End If
    Call MessageBox(Message & vbCrLf & vbCrLf & "xmMP3Err: " _
    & ErrorNum & vbCrLf, vbCritical, Titolo)
    Call ScriviLOG(Dove, "xmMP3Err: " + CStr(ErrorNum), Message)
ErrH:
    If Err.Number <> 0 Then
         Call MessageBox(Err.Number & vbCrLf & Err.Source & _
                vbCrLf & "modErrorHandler.DisplayError" & vbCrLf & Err.Description)
    End If
End Sub
