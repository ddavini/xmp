...
'////////////////////////////////////////////////////////////////////////////////
' frmRecTest.frm - Copyright (c) 2002-2005 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                        [http://www.jobnik.org]
'                                                        [  jobnik@jobnik.org  ]
'
...
Private Sub tmrRecTest_Timer()
    ...
    If (rchan) Then 'recording/encoding
        lblPos.Caption = BASS_ChannelGetPosition(rchan)
    ElseIf (chan) Then
        If (BASS_ChannelIsActive(chan)) Then 'playing
            lblPos.Caption = BASS_ChannelGetPosition(chan) & " / " & BASS_ChannelGetLength(chan)
        Else
            lblPos.Caption = BASS_ChannelGetLength(chan)
        End If
    End If
End Sub
