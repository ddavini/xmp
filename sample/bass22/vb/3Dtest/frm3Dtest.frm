...
'
' Updated: 2003-2005 by (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                     [http://www.jobnik.org]
'                                     [  jobnik@jobnik.org  ]
'
...
Private Type channel
    ...
    dir_ As Integer         'direction of the channel
End Type
...
Private Sub Form_Load()
    ...
    If (BASS_Init(frmDevice.device, 44100, BASS_DEVICE_3D, Me.hWnd, 0) = 0) Then
...
Sub UpdateButtons()
    ...
    For i = 0 To 4
        optDirection(i).Enabled = IIf(chan = -1, False, True)
    Next i

    If chan <> -1 Then optDirection(chans(chan).dir_).value = True
End Sub
...
Private Sub cmdAdd_Click()
    ...
    If newchan = 0 Then newchan = BASS_SampleLoad(BASSFALSE, DLG.FileName, 0, 0, 1, BASS_SAMPLE_LOOP Or BASS_SAMPLE_3D)

    If newchan Then
        ReDim Preserve chans(chanc) As channel
        chans(chanc).channel = newchan
        chans(chanc).dir_ = ID_NONE
...
Private Sub ID_Rolloff_Scroll()
    ...
    Call BASS_Set3DFactors(-1#, pow(2#, (ID_Rolloff.value - 10) / 5#), -1#)
End Sub

Private Sub ID_Doppler_Scroll()
    'Change the doppler factor
    Call BASS_Set3DFactors(-1#, -1#, pow(2#, (ID_Doppler.value - 10) / 5#))
End Sub
...
Private Sub optDirection_Click(index As Integer)
    If chan = -1 Then Exit Sub
    Select Case index
        Case ID_LEFT
            chans(chan).dir_ = ID_LEFT
...
'Calculate power of
Public Function pow(ByVal a As Integer, ByVal b As Integer) As Long
   Dim i As Integer
