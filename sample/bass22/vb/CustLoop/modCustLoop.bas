...
'/////////////////////////////////////////////////////////////////////////////////
' modCustLoop.bas - Copyright (c) 2004-2005 (: JOBnik! :) [Arthur Aminov, ISRAEL]
'                                                         [http://www.jobnik.org]
'                                                         [  jobnik@jobnik.org  ]
...
Sub LoopSyncProc(ByVal handle As Long, ByVal channel As Long, ByVal data As Long, ByVal user As Long)
    If (BASS_ChannelSetPosition(channel, loop_(0)) = 0) Then 'try seeking to loop start
        Call BASS_ChannelSetPosition(channel, 0) 'failed, go to start of file instead
    End If
End Sub
...
Sub ScanPeaks(ByVal decoder As Long)
    ...
    Do While (Not killscan)
        ...
        pos = BASS_ChannelGetPosition(decoder) / bpp
...
Function PlayFile() As Boolean
    ...
    With frmCustLoop.cmdCustLoop
        ...
        chan = BASS_StreamCreateFile(BASSFALSE, .FileName, 0, 0, 0)
        If (chan = 0) Then chan = BASS_MusicLoad(BASSFALSE, .FileName, 0, 0, BASS_MUSIC_RAMPS Or BASS_MUSIC_POSRESET Or BASS_MUSIC_PRESCAN, 0)
        ...
        bpp = BASS_ChannelGetLength(chan) / WIDTH_ 'bytes per pixel
        ...
        Dim chan2 As Long
        chan2 = BASS_StreamCreateFile(BASSFALSE, .FileName, 0, 0, BASS_STREAM_DECODE)
        If (chan2 = 0) Then chan2 = BASS_MusicLoad(BASSFALSE, .FileName, 0, 0, BASS_MUSIC_DECODE, 0)
