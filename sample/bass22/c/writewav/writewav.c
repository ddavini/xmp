// BASS Simple Console WAVE Writer, copyright (c) 2002-2005 Ian Luck.
...
void main(int argc, char **argv)
{
	...
	if ((chan=BASS_StreamCreateFile(FALSE,argv[1],0,0,BASS_STREAM_DECODE))
		...
		printf("playing MOD music \"%s\" [%u orders]",BASS_MusicGetName(chan),BASS_MusicGetOrders(chan));
		pos=BASS_ChannelGetLength(chan);
	}
...
#ifdef BIG_ENDIAN // swap byte order
