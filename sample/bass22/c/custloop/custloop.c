// BASS custom looping example, copyright (c) 2004-2005 Ian Luck.
...
void CALLBACK LoopSyncProc(HSYNC handle,DWORD channel,DWORD data,DWORD user)
{
	if (!BASS_ChannelSetPosition(channel,loop[0])) // try seeking to loop start
...
void __cdecl ScanPeaks(DWORD decoder)
{
	...
	while (!killscan) {
		...
		DWORD pos=BASS_ChannelGetPosition(decoder)/bpp;
		...
		else pos=BASS_ChannelGetPosition(decoder)/bpp;
		if (pos>cpos) {
			DWORD a;
...
BOOL PlayFile()
{
	...
	}
	{
		...
		// create the bitmap
		wavebmp=CreateDIBSection(0,(BITMAPINFO*)bh,DIB_RGB_COLORS,&wavebuf,NULL,0);
		wavedc=CreateCompatibleDC(0);
		SelectObject(wavedc,wavebmp);
	}
	bpp=BASS_ChannelGetLength(chan)/WIDTH; // bytes per pixel
	...
	BASS_ChannelPlay(chan,FALSE); // start playing
	{
		...
		scanthread=_beginthread(ScanPeaks,0,chan2); // start scanning peaks in a new thread
	}
	return TRUE;
}
	...
	switch (m) {
		...
		case WM_PAINT:
			if (GetUpdateRect(h,0,0)) {
				...
				DrawTimeLine(dc,BASS_ChannelGetPosition(chan),0xffffff,0); // current pos
				EndPaint(h,&p);
			}
			return 0;
