// BASS Console Test (LL/GPA version), copyright (c) 1999-2005 Ian Luck.
...
#define LOADBASSFUNCTION(f) *((void**)&f)=GetProcAddress(bass,#f)
	...
	LOADBASSFUNCTION(BASS_MusicGetName);
...
void main(int argc, char **argv)
{
	...
	if ((chan=BASS_StreamCreateFile(FALSE,argv[1],0,0,BASS_SAMPLE_LOOP))
		...
		printf("playing MOD music \"%s\" [%u chans, %u orders]",BASS_MusicGetName(chan),a,BASS_MusicGetOrders(chan));
		pos=BASS_ChannelGetLength(chan);
		ismod=TRUE;
	}
