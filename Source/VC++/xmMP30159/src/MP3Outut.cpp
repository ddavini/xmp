...
//	memcpy(DeviceName,&woc.szPname,sizeof(woc.szPname));
...
DWORD output_getCurrentSample()
{
	...
	Sample = output_bytesToSample(Sample) * m_stepPitch;
...
int output_getSpectrum(DWORD sample, int* pSpecL, int* pSpecR)
{
	...
	long bytes = output_sampleToBytes(sample);
	int minBytes, maxBytes, lost;
	int index, pos;
...
void output_playBlock()
{
	...
	if ( m_bitsPerSample == 16 ) {
		switch ( m_xsoundmode ) {
			case 1:
				output_xsound((short*) pWaveHdr->lpData, pWaveHdr->dwBufferLength );
				break;
			case 2:
				output_xnormalize((short*) pWaveHdr->lpData, pWaveHdr->dwBufferLength);
				break;
			case 3:
				output_xsound((short*) pWaveHdr->lpData, pWaveHdr->dwBufferLength );
				output_xnormalize((short*) pWaveHdr->lpData, pWaveHdr->dwBufferLength);
...
void output_xnormalize( short *buf, int size )
{
	...
	if (m_xnormset > 0) {
		normalize = m_xnormset;
		output_applynormalize(buf,size,normalize);
		m_xnorm = output_calcnormalize(buf,size);
	} else {
		normalize = output_calcnormalize(buf,size);
		m_xnorm = normalize; 
		output_applynormalize(buf,size,normalize);
	}
    /////////////////////
...
double output_calcnormalize( short *buf, int size )
{
	...
	double maxvalue = 0;
		...
		size = (int)(size / sizeof(short));
		areamax = 32767 * size;
		max = (areamax / 100) * m_xsoundoption;
	//	max = areamax - max;
		...
		if (maxvalue > max) {
			normalize = max / maxvalue;
			if (m_xnorm > normalize) { 
				return normalize;
			}
