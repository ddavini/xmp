...
Public Sub DisplayError(Message As String, Optional Tipo As VbMsgBoxStyle = vbCritical, _
                        ...
                        Optional Dove As String = "Error", _
                        Optional ListINFO As Boolean = False)
...
On Error GoTo ErrH
    ...
    aErr = Split(Message, vbCrLf)
    If Not fServer Then
        Call MessageBox(aErr(UBound(aErr)), vbCritical, Titolo, xmp)
    End If
    
    If ListINFO Then
        WriteINFO aErr(UBound(aErr))
    End If
